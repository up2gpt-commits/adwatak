...
export async function POST(req: NextRequest) {
  ...
  const token = extractToken(req, "/api/seo-audit");
  const tv = verifyToken(token, "/api/seo-audit", req);
  if (!tv.ok) {
    console.warn(`[api:seo-audit] token rejected:`, tv.reason);
  ...
  // Rate limiting: 6 req / 60s per IP
  const rl = rateLimit({ key: `api:seo-audit:${getClientIp(req)}`, max: 6, windowSec: 60 });
  if (!rl.allowed) {
    return NextResponse.json(
