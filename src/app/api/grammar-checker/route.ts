...
export async function POST(req: NextRequest) {
  ...
  const token = extractToken(req, "/api/grammar-checker");
  const tv = verifyToken(token, "/api/grammar-checker", req);
  if (!tv.ok) {
    console.warn(`[api:grammar-checker] token rejected:`, tv.reason);
