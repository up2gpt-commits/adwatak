...
const LANG = "ar";
const T = {
  title: "أداة البحث عن الكلمات المفتاحية",
  subtitle: "احصل على كلمات مفتاحية دقيقة لتحسين محركات البحث (SEO)",
  inputLabel: "الكلمة المفتاحية أو الموضوع",
  inputPlaceholder: "مثال: تسويق عقاري، حاسبة القرض، ...",
  searchBtn: "🔍 بحث",
  searching: "⏳ جاري التحليل...",
  results: "نتائج الكلمات المفتاحية",
  noResults: "لم نتمكن من توليد كلمات مفتاحية. جرب كلمة أخرى.",
  ...
  avgDifficulty: "متوسط الصعوبة",
  easyWins: "أسهل الكلمات للترتيب",
  topRec: "أفضل توصية",
  summary: "ملخص",
  share: "مشاركة",
};
...
export default function Client() {
  ...
  async function handleSearch() {
    ...
    try {
      const r = await fetch("/api/keyword-research", {
        ...
        body: JSON.stringify({ keyword: keyword.trim(), lang: "ar" }),
      });
  ...
  return (
    <div className="max-w-[820px] mx-auto">
      <StructuredData data={toolSchema("keyword-research", "أداة البحث عن الكلمات المفتاحية", "https://adwatak.cloud/tools/keyword-research", "ar")} />
      <Breadcrumb lang="ar" category="أدوات SEO" categorySlug="seo" toolName="أداة الكلمات المفتاحية" />
      ...
      {loading && (
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center mb-6">
          ...
          <p className="text-gray-500">جاري تحليل الكلمات المفتاحية...</p>
        </div>
      )}
        ...
        <>
          ...
          {result.analysis && (
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border border-blue-100 p-6 mb-6">
              ...
              {result.analysis.easyWins && result.analysis.easyWins.length > 0 && (
                <div className="mb-3">
