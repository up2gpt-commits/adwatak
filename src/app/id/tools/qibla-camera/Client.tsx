    ...
    const draw = (time: number) => {
      ...
      if (currentHeading >= 0 && diff >= 0) {
        ...
        if (!isAligned) {
          ...
          const turnText = Math.abs(turnAngle) < 5 ? "Go straight" : turnAngle > 0 ? "Belok kanan" : "Belok kiri";
      ...
      {camError && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/80">
          <div className="text-center p-6">
            ...
            <button onClick={onBack} className="bg-white/20 hover:bg-white/30 text-white px-6 py-3 rounded-xl transition-colors">Kembali</button>
          </div>
        </div>
      )}

      <button onClick={() =>{ headingRef.current = -1; onBack(); }} className="absolute top-4 right-4 z-10 bg-black/30 hover:bg-black/50 text-white px-4 py-2 rounded-xl transition-colors text-sm">✕ Kembali</button>
...
const seoContent = [
  `Alat kamera Qibla menampilkan panah hijau di atas tampilan kamera langsung untuk menunjukkan arah Ka'bah. Pengguna dapat melihat lingkungan sekitar sambil mengetahui posisi Ka'bah secara akurat.`,
  `Aplikasi ini menggunakan data GPS dan sensor kompas untuk menghitung jalur great-circle menuju Makkah. Perhitungan geometri bumi memastikan arah Qibla sesuai dengan posisi pengguna saat ini.`,
  `Kalibrasi sensor dilakukan dengan menggerakkan ponsel membentuk angka delapan di udara. Hindari benda logam saat kalibrasi dan lakukan ulang setiap kali berpindah kota untuk menjaga akurasi.`,
  `Penggunaan kamera Qibla menghabiskan baterai sekitar enam hingga delapan persen setiap lima belas menit. Kurangi kecerahan layar dan tutup aplikasi setelah digunakan untuk menghemat daya baterai.`,
  `Alat ini kompatibel dengan iPhone 8 ke atas dan perangkat Android 8.0 ke atas. Pastikan ponsel memiliki sensor giroskop agar panah arah dapat bergerak sesuai orientasi perangkat.`,
];

const faqs = [
  { question: "Seberapa akurat kamera penunjuk qibla ini?", answer: "Akurasi bergantung pada sensor kompas dan GPS perangkat Anda, biasanya memiliki deviasi 2-5 derajat. Untuk hasil terbaik, kalibrasi kompas secara rutin dan gunakan di area terbuka jauh dari gangguan logam." },
  { question: "Apakah aplikasi ini bisa digunakan offline?", answer: "Ya, setelah lokasi Anda tersimpan, aplikasi dapat menentukan arah qibla tanpa koneksi internet. Namun, GPS masih diperlukan untuk lokasi yang akurat jika Anda berpindah tempat." },
  { question: "Bagaimana cara mengkalibrasi kompas?", answer: "Kalibrasi biasanya dilakukan dengan menggerakkan perangkat membentuk angka delapan di udara atau memutar ponsel ke segala arah beberapa kali. Ikuti petunjuk yang muncul di layar jika tersedia." },
  { question: "Apakah bisa digunakan di Android dan iPhone?", answer: "Ya, aplikasi kamera qibla ini kompatibel dengan perangkat Android dan iOS (iPhone) yang memiliki sensor kompas (magnetometer) dan kamera." },
  { question: "Apakah GPS harus diaktifkan?", answer: "Ya, GPS perlu diaktifkan untuk menentukan koordinat lokasi Anda secara akurat. Tanpa GPS, aplikasi mungkin menggunakan jaringan yang kurang tepat atau tidak bisa menentukan arah qibla dengan benar." },
  { question: "Mengapa panah terus berputar dan tidak stabil?", answer: "Ini biasanya terjadi karena gangguan medan magnet dari benda logam, perangkat elektronik, atau kompas yang belum terkalibrasi. Coba kalibrasi ulang kompas dan pindah ke lokasi terbuka jauh dari logam." },
  { question: "Apa bedanya dengan aplikasi kompas biasa?", answer: "Aplikasi ini secara otomatis menghitung arah qibla berdasarkan koordinat Ka'bah dari lokasi Anda, bukan hanya menunjukkan utara magnetik. Selain itu, tampilan kamera AR memudahkan Anda melihat arah secara langsung." },
  { question: "Apakah deklinasi magnet mempengaruhi akurasi?", answer: "Ya, deklinasi magnet (selisih antara utara magnetik dan utara geografis) dapat mempengaruhi hasil. Aplikasi ini biasanya sudah mengkompensasi deklinasi berdasarkan lokasi GPS Anda, namun gangguan lokal tetap bisa memengaruhi." },
  { question: "Apakah aplikasi ini boros baterai?", answer: "Penggunaan kamera, GPS, dan sensor kompas secara bersamaan memang membutuhkan daya lebih tinggi dari biasanya. Disarankan untuk mengisi daya baterai atau menutup aplikasi setelah selesai digunakan." },
  { question: "Bolehkah digunakan di dalam ruangan dekat benda logam?", answer: "Tidak disarankan. Benda logam, beton bertulang, dan perangkat elektronik di dalam ruangan dapat mengganggu medan magnet dan membuat penunjuk arah menjadi tidak akurat. Gunakan di area terbuka untuk hasil terbaik." },
];
...
const relatedTools = [
  ...
  { title: "Tasbih Digital", icon: "📿", href: "/id/tools/tasbeeh-counter" },
  { title: "Kalkulator Zakat", icon: "💰", href: "/id/tools/zakat-calculator" },
  { title: "Fidyah Kaffarah", icon: "🌲", href: "/id/tools/fidyah-kaffarah" },
];
...
export default function Client({ locale = "en" }: { locale?: "ar" | "en" | "tr" | "id" }) {
  ...
  const handleGPS = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setErrorMsg("Tidak dapat menentukan lokasi Anda."); setMode("error"); },
  ...
  const handleManual = () => {
    ...
    if (isNaN(lat) || isNaN(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
      setErrorMsg("Koordinat tidak valid"); setMode("error"); return;
    }
    showResult(lat, lng);
  };
  ...
  return (
    <div className="max-w-[760px] mx-auto">
      <div className="bg-white rounded-2xl border border-gray-200 p-8 mb-6">
        <h1 className="text-2xl font-extrabold mb-1">📸 Pencari Kamera Kiblat</h1>
        <p className="text-sm text-gray-500 mb-6">Arahkan kamera Anda ke arah Kiblat dan lihat Ka'bah!</p>
        ...
        {mode === "city" && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={handleCity} disabled={!selectedCity} className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold px-6 py-3 rounded-xl transition-colors">Buka Kamera</button>
              <button onClick={reset} className="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold px-6 py-3 rounded-xl">Kembali</button>
            </div>
          </div>
        )}
        ...
        {mode === "manual" && (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Lintang:</label>
              <input type="number" step="any" value={manualLat} onChange={(e) => setManualLat(e.target.value)} placeholder="mis. 30.0444" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-amber-500 outline-none" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Bujur:</label>
              <input type="number" step="any" value={manualLng} onChange={(e) => setManualLng(e.target.value)} placeholder="mis. 31.2357" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-amber-500 outline-none" />
            </div>
            <div className="flex gap-3">
              <button onClick={handleManual} disabled={!manualLat || !manualLng} className="flex-1 bg-amber-600 hover:bg-amber-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold px-6 py-3 rounded-xl transition-colors">Buka Kamera</button>
              <button onClick={reset} className="bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold px-6 py-3 rounded-xl">Kembali</button>
            </div>
          </div>
        )}
