    ...
    <div className="flex flex-col items-center">
      ...
      <div className="mt-3 text-center">
        <span className="inline-block bg-red-100 text-red-800 font-bold px-4 py-2 rounded-full text-sm">
          🧭 Qibla: {direction.en} ({bearing.toFixed(1)}°)
        </span>
      </div>
    </div>
  );
...
export default function Client() {
  ...
  // Try to get device orientation for compass (with iOS permission handling)
  useEffect(() => {
  ...
  const cityName = selectedCity ? CITIES[selectedCity]?.en || "" : "";

  return (
    <div className="max-w-[760px] mx-auto">
      <StructuredData data={toolSchema("Qibla Direction", "Find the Qibla direction from anywhere in the world — interactive compass with precise Haversine calculation", "https://adwatak.cloud/en/tools/qibla-direction", "en", "Islamic")} />
          ...
          <div className="text-center">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 max-w-sm mx-auto">
              <div className="bg-green-50 border border-green-200 rounded-xl p-3">
                ...
                <p className="text-lg font-extrabold text-green-800">{direction.en}</p>
              </div>
            ...
            {relDirection && (
              <div className="mt-4 bg-red-50 border border-red-200 rounded-xl p-4 max-w-sm mx-auto">
                <p className="text-sm font-bold text-red-800">
                  📍 Qibla is {relDirection.en} — point your device forward and follow the red arrow
                </p>
              </div>
            )}
        ...
        {mode === "idle" && (
          <div className="space-y-4">
            ...
            >
              ...
              <div>
                <p className="font-bold">Auto-detect my location (GPS)</p>
                <p className="text-xs opacity-80">Most accurate — uses your real GPS position</p>
              </div>
            </button>
            ...
            >
              ...
              <div>
                <p className="font-bold">Select city from list</p>
                <p className="text-xs opacity-80">Over 60 cities in 35+ countries</p>
              </div>
            </button>
            ...
            >
              <span className="text-2xl">📐</span>
              <div>
                <p className="font-bold">Enter coordinates manually</p>
                <p className="text-xs opacity-80">Type latitude and longitude yourself</p>
              </div>
            </button>
          </div>
        )}
            ...
            <div>
              ...
              >
                ...
                {Object.entries(COUNTRIES).map(([code, c]) => (
                  <option key={code} value={code}>{c.en}</option>
                ))}
              </select>
            </div>
              ...
              <div>
                ...
                >
                  ...
                  {COUNTRIES[selectedCountry]?.cities.map((key) => (
                    <option key={key} value={key}>{CITIES[key].en}</option>
                  ))}
                </select>
              </div>
            )}
