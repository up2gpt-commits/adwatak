...
{title:"Plagiarism Checker",icon:"🚫",href:"/fr/tools/plagiarism-checker"},
{title:"AI Content Detector",icon:"🤖",href:"/fr/tools/ai-content-detector"},
...
const check=async()=>{if(text.trim().length<10){setError("Please enter text");return;}setError("");setLoading(true);setResult(null);try{const res=await fetch("/api/grammar-checker",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:text.trim(),lang:"fr"})});if(!res.ok)throw new Error((await res.json()).error||"Check failed");setResult(await res.json());}catch(e:any){setError(e.message||"Error occurred.");}finally{setLoading(false);}};
