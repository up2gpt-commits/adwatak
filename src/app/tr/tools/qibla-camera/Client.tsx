...
function ARCamera({ lat, lng, bearing, onBack, locale }: { lat: number; lng: number; bearing: number; onBack: () => void; locale: "ar" | "en" | "tr" | "id" }) {
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-50 bg-black overflow-hidden">
      ...
      <button onClick={() => { headingRef.current = -1; onBack(); }} className="absolute top-4 right-4 z-10 bg-black/30 hover:bg-black/50 text-white px-4 py-2 rounded-xl transition-colors text-sm">✕ Geri</button>
...
const seoContent = [
  `Canlı kamera görüntüsü üzerinde Kâbe yönünü gösteren yeşil bir ok işareti belirir. Bu artırılmış gerçeklik özelliği sayesinde telefonunuzu kaldırmanız yeterlidir, nerede olursanız olun kıble yönünü anında görürsünüz.`,
  `Uygulama GPS konumunuzu ve cihazınızın pusula sensörünü kullanarak Mekke'deki Kâbe'ye olan en kısa büyük daire rotasını hesaplar. Bu hesaplama anlık olarak güncellenir ve hareket ettikçe ok işareti sizi doğru yöne yönlendirir.`,
  `En doğru sonuç için telefonunuzu havada sekiz şeklinde çizerek hareket ettirin ve pusulayı kalibre edin. Metal nesnelerden, araçlardan ve betonarme yapılardan en az 1 metre uzak durun, aksi halde manyetik alan bozulabilir.`,
  `Kamera ve GPS aktif olduğu için her 15 dakikada pilin yaklaşık %6-8'i tükenir. Pil ömrünü uzatmak için ekran parlaklığını düşürebilir veya kullanmadığınız uygulamaları kapatabilirsiniz.`,
  `iPhone 8 ve üzeri modeller ile Android 8.0 ve sonrası işletim sistemine sahip telefonlarla uyumludur. Cihazınızda jiroskop sensörü bulunması en iyi performans için önerilir.`,
];

const faqs = [
  { question: "Kıble kamera bulucunun doğruluğu nedir?", answer: "Cihazınızın sensör kalitesine bağlı olarak genellikle 2-5 derece hassasiyetle çalışır. Manyetik alanlardan uzak, düz bir zeminde daha doğru sonuç alırsınız." },
  { question: "İnternet olmadan çalışır mı?", answer: "Evet, temel kıble bulma ve kamera özellikleri çevrimdışı çalışır. Ancak ilk konum tespiti ve manyetik sapma düzeltmesi için bir kez internete ihtiyaç duyabilir." },
  { question: "Pusula nasıl kalibre edilir?", answer: "Telefonunuzu 8 şeklinde sallayarak veya ekrandaki talimatları takip ederek kalibre edebilirsiniz. Kalibrasyonu metal nesnelerden uzakta, düz bir zeminde yapmanız önerilir." },
  { question: "Hem Android hem de iPhone'da çalışır mı?", answer: "Evet, Android ve iOS (iPhone) işletim sistemlerinin tamamında sorunsuz çalışır. Tek gereksinim, cihazınızda manyetik sensör (pusula) ve kamera bulunmasıdır." },
  { question: "GPS açık olması gerekli mi?", answer: "En doğru kıble açısı için GPS'in açık olması önerilir. GPS kapalıyken son bilinen konum veya Wi-Fi bazlı konum kullanılabilir, ancak doğruluk düşebilir." },
  { question: "Ok sürekli dönüyorsa ne yapmalıyım?", answer: "Önce cihazınızı kalibre edin. Metal kılıf, telefon tutucu veya elektronik cihazlardan uzaklaşın. Sorun devam ederse telefonu yeniden başlatmayı deneyin." },
  { question: "Normal pusula uygulamalarından farkı nedir?", answer: "Standart pusula uygulamaları sadece kuzeyi gösterir. Bu araç ise kameranız üzerinden artırılmış gerçeklik (AR) ile bulunduğunuz konuma özel Kâbe yönünü doğrudan ekranda gösterir." },
  { question: "Manyetik sapma doğruluğu etkiler mi?", answer: "Evet, manyetik kuzey ile coğrafi kuzey arasındaki fark (manyetik sapma) kıble açısını değiştirebilir. Uygulama konumunuza göre bu sapmayı otomatik olarak düzelterek daha isabetli sonuç sunar." },
  { question: "Pil tüketimi nasıldır?", answer: "Kamera ve sensörlerin aktif kullanımı nedeniyle normalden biraz daha fazla pil harcar. Günlük kısa süreli kullanımda etkisi azdır; uzun süreli kullanımda şarj aleti önerilir." },
  { question: "Metal yakınında ve iç mekânda kullanabilir miyim?", answer: "Metal yapılar, betonarme duvarlar ve elektronik cihazlar manyetik sensörü bozabilir. Mümkünse açık alanda veya pencere kenarında kullanın. İç mekânda kalibrasyon yaparak deneyebilirsiniz ancak sonucu kontrol etmeniz önerilir." },
];

const relatedTools = [
  { title: "Kibla Yonu", icon: "🧭", href: "/tr/tools/qibla-direction" },
  { title: "Hicri Takvim", icon: "📅", href: "/tr/tools/hijri-converter" },
  { title: "Namaz Vakitleri", icon: "🕌", href: "/tr/tools/prayer-times" },
  { title: "Tesbih Sayaci", icon: "📿", href: "/tr/tools/tasbeeh-counter" },
  { title: "Zekat Hesaplama", icon: "💰", href: "/tr/tools/zakat-calculator" },
  { title: "Fidye Keffaret", icon: "🌲", href: "/tr/tools/fidyah-kaffarah" },
];

export default function Client({ locale = "tr" }: { locale?: "ar" | "en" | "tr" | "id" }) {
  ...
  return (
    <div className="max-w-[760px] mx-auto">
      <div className="bg-white rounded-2xl border border-gray-200 p-8 mb-6">
        <h1 className="text-2xl font-extrabold mb-1">📸 Kıble Kamera Bulucu</h1>
        ...
        {mode === "city" && (
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              <button onClick={handleCity} disabled={!selectedCity} className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold px-6 py-3 rounded-xl transition-colors">Kamerayı Aç</button>
        ...
        {mode === "manual" && (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Enlem:</label>
              <input type="number" step="any" value={manualLat} onChange={(e) => setManualLat(e.target.value)} placeholder="örn. 30.0444" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-amber-500 outline-none" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Boylam:</label>
              <input type="number" step="any" value={manualLng} onChange={(e) => setManualLng(e.target.value)} placeholder="örn. 31.2357" className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-amber-500 outline-none" />
            </div>
            <div className="flex gap-3">
